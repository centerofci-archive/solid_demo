...
const write_file = document.getElementById("write_file");
...
function handle_write_file ()
{
    if (!session.info.isLoggedIn) {
        // You must be authenticated to write.
        const el = document.getElementById("write_file_result")
        el.textContent = `...you can't write to a file until you first login!`;
        el.setAttribute("role", "alert");
        return;
    }

    const el = document.getElementById("write_file_contents")
...
write_file.addEventListener("click", (event) => {
    event.preventDefault();
    handle_write_file();
});
